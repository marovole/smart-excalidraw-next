...
### JSON 输出规范
1. **必须在单个 \`\`\`json ... \`\`\` 代码块中输出完整的 Excalidraw JSON**，代码块前后不要添加任何解释性文字或描述。
2. **严格遵循标准 JSON 语法**：
   - 所有属性名（键）必须使用双引号包裹
   - 字符串值必须使用双引号，不能使用单引号
   - 不能包含任何 JavaScript 风格的注释（// 或 /* */）
   - 不能有尾随逗号（数组或对象最后一项后不能有逗号）
3. **禁止在代码块外输出任何内容**，生成的结果应该可以直接被 JSON.parse() 解析，无需额外处理。
...
export const USER_PROMPT_TEMPLATE = (userInput, chartType = 'auto') => {
  ...
  if (chartType && chartType !== 'auto') {
    ...
    if (chartTypeName) {
      ...
      // Add JSON format reminder for chart type mode
      promptParts.push(
        '⚠️ 输出格式提醒：请只在单个 ```json ... ``` 代码块中返回标准 JSON 格式，所有键必须使用双引号，不要输出注释、尾随逗号、单引号或额外的自然语言描述。'
      );
    }
  } else {
    ...
    // Add JSON format reminder for auto mode
    promptParts.push(
      '⚠️ 输出格式提醒：输出内容必须封装在单个 ```json ... ``` 代码块中，使用标准 JSON 语法（键使用双引号、无单引号/注释/尾随逗号），不要在代码块外添加额外文字说明。'
    );
  }
